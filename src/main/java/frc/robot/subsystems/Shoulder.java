...
public class Shoulder extends SubsystemBase {
    ...
    private final SysIdRoutine m_SysIdRoutine = new SysIdRoutine(
        ...
        new SysIdRoutine.Mechanism(
            volts -> shoulderMotor.setControl(m_sysIdControl.withOutput(volts)),
            null,
            this
        )
    );
    ...
    */
    ...
    public Command sysIdQuasistatic(SysIdRoutine.Direction direction) {
        if (direction == Direction.kForward) {
            return m_SysIdRoutine.quasistatic(direction).until(() -> shoulderMotor.getPosition().getValueAsDouble() > 185);//tune based off mechanism to not hit hardstop
        } else
            return m_SysIdRoutine.quasistatic(direction).until(() -> shoulderMotor.getPosition().getValueAsDouble() < 1.5);
    } 
    

    public Command sysIdDynamic(SysIdRoutine.Direction direction) {
        if (direction == Direction.kForward) {
            return m_SysIdRoutine.dynamic(direction).until(() -> shoulderMotor.getPosition().getValueAsDouble() > 185);//account for extra errors from inertia
        } else
            return m_SysIdRoutine.dynamic(direction).until(() -> shoulderMotor.getPosition().getValueAsDouble() < 1.5);
    }
